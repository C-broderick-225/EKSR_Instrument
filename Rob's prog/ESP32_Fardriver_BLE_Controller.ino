 ...
 *
 * This is a stripped-down version for sharing on forums.
 ...
 *
 * Removed all display-related code, buttons, and persistent storage.
 *
 ****************************************************************************/
...
void processPacket(uint8_t* packet, size_t length) {
  ...
  switch (address) {
    case 0xE2:
      ...
      break;
    case 0xE8: {
      float new_voltage = (((pData[1] << 8) | pData[0]) / 10.0f) - 0.8f;
      ...
      if (new_lineCurrent >= -140.0f && new_lineCurrent <= 140.0f) { 
    ...
    case 0xD6: {
      ...
      if (new_controllerTemp > -20 && new_controllerTemp < 150) { controllerData.controllerTemp = new_controllerTemp; }
      break;
    }
    case 0xF4: {
      int new_motorTemp = (int16_t)((pData[1] << 8) | pData[0]);
      if (new_motorTemp > -20 && new_motorTemp < 150) { controllerData.motorTemp = new_motorTemp; }
  ...
  if (lastPacketTime > 0 && currentTime > lastPacketTime) {
    float deltaTime_s = (currentTime - lastPacketTime) / 1000.0f;
    float distance_delta_km = controllerData.speed * (deltaTime_s / 3600.0f);
    controllerData.trip += distance_delta_km;
    controllerData.odo += distance_delta_km;
  }
...
void printControllerData() {
  ...
  Serial.print("Trip: "); Serial.print(controllerData.trip, 1); Serial.println(" km");
  Serial.print("Odo: "); Serial.print(controllerData.odo, 1); Serial.println(" km");
